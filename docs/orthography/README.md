...
Joining what the decode separated overrides nothing, since the dictionary had no
entry for the joined form and there was no evidence either way. Splitting a
...
The repeat rule, as in 研究研究 and 休息休息, reads two words rather than one, because
...
where CC-CEDICT writes the headword's pinyin in lower case. There is no
...
巴赫 and 牛顿 apart, not one of which is a Chinese name, since the shape is
identical to 齐 + 白石. And a list of generics cannot say where 上海浦东发展银行 divides.
...
unreliable here, since the phrase corpus writes 玩儿 as `wán er`, which is why
