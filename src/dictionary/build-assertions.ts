/**
 * What must hold of every build, as a list.
 *
 * Each entry stands for a whole class of upstream defect that a source
 * refresh could reintroduce, so a failure here means the merge no longer
 * repairs something it used to. `assertions.ts` runs them.
 */
