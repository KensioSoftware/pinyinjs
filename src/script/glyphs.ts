...
import { splitFormAt, splitFormsOf } from "./reading-splits.js";

export { isReadingScoped, isReadingSensitive } from "./reading-splits.js";
...
export function toRegionalGlyph(
  ...
  return (
    splitFormAt(character, syllable) ??
    HONG_KONG_FORMS.get(character) ??
    character
  );
}

/**
 * Every form a character can take in a region, likeliest first.
 *
 * The region's own answer followed by whatever a reading would have kept, which
 * is what a conversion made without a reading needs in order to say what it was
 * choosing between.
 */
export function regionalGlyphsOf(
  character: string,
  region: Region,
): readonly string[] {
  return [
    ...new Set([
      toRegionalGlyph(character, region),
      ...splitFormsOf(character),
    ]),
  ];
}
